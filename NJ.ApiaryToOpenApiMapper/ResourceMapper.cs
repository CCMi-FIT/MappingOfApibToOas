...
namespace NJ.ApiaryToOpenApiMapper
{
  public static class ResourceMapper
  {
    ...
    public static PathsObject MapResources(ApiaryApiBlueprint apb, IReadOnlyCollection<ApiType> apiaryNamedTypes)
    {
      var result = new PathsObject
      {
        PathItems = ResourceMapper.MapResourcesToDictionary(apb, apiaryNamedTypes)
      };
      return result;
    }

    public static IReadOnlyDictionary<string, PathItemObject> MapResourcesToDictionary(ApiaryApiBlueprint apb, IReadOnlyCollection<ApiType> apiaryNamedTypes)
    {
      ...
      var operationInfos = resourceInfos.SelectMany(ri => MapResourceSectionToOperationInfo(ri, apiaryNamedTypes));
    ...
    private static IEnumerable<OperationInfo> MapResourceSectionToOperationInfo(ResourceInfo resourceInfo, IReadOnlyCollection<ApiType> apiaryNamedTypes)
    {
      ...
      var result = resourceSection.ActionSections.Select(a =>  MapActionSectionToOperationInfo(a, resourceInfo, apiaryNamedTypes));
      return result;
    }

    private static OperationInfo MapActionSectionToOperationInfo(ActionSection actionSection, ResourceInfo resourceInfo, IReadOnlyCollection<ApiType> apiaryNamedTypes)
    {
      var resourceSection = resourceInfo.ResourceSection;
      ...
      var operationObject = ActionSectionMapper.Map(actionSection, resourceSection, resourceGroupSection, apiaryNamedTypes);
