...
namespace NJ.ApiaryToOpenApiMapper
{
  ...
  {
    public static OperationObject Map(ActionSection actionSection, ResourceSection resourceSection, ResourceGroupSection resourceGroupSection, IReadOnlyCollection<ApiType> apiaryNamedTypes)
    {
      var (requestBodyObject, requestBodyParameters) = MapRequestSections(actionSection, apiaryNamedTypes);
      var responseSectionsGroupedByStatusCode = actionSection.ResponseSections.GroupBy(s => s.HttpStatusCode);
      var statusCodesWithResponseObjects = responseSectionsGroupedByStatusCode.ToDictionary(g => g.Key.ToString(), g => (IResponseOrReferenceObject)MapToResponseObject(g.Key, g, apiaryNamedTypes));
    ...
    private static ResponseObject MapToResponseObject(int statusCode, IEnumerable<ResponseSection> responseSections, IReadOnlyCollection<ApiType> apiaryNamedTypes)
    {
      ...
      var result = new ResponseObject
      {
        ...
        Content = responseSectionCollection.Where(r => r.MediaType is not null).ToDictionary(r => r.MediaType, r => MapperToMediaTypeObject.MapToMediaTypeObject(r.MediaType, r.BodySection?.Content, r.SchemaSection, r.AttributesSection, apiaryNamedTypes, false))
      };
    ...
    public static (RequestBodyObject RequestBodyObject, IList<IParameterOrReferenceObject> ParameterObjects) MapRequestSections(ActionSection actionSection, IReadOnlyCollection<ApiType> apiaryNamedTypes)
    {
      ...
      var mediaTypesWithMediaTypeObjects = requestSections.Where(r => r.MediaType is not null).ToDictionary(r => r.MediaType, r => MapRequestSection(r, actionSection, apiaryNamedTypes));
    ...
    public static MediaTypeObject MapRequestSection(RequestSection requestSection, ActionSection actionSection, IReadOnlyCollection<ApiType> apiaryNamedTypes)
    {
      // TODO: Examples?
      ...
      var result = MapperToMediaTypeObject.MapToMediaTypeObject(requestSection.MediaType, content, requestSection.SchemaSection, attributesSection, apiaryNamedTypes, true);
      return result;
    }
  }
}